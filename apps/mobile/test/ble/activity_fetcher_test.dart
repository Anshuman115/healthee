...
Uint8List _allSentinel(int bytes) => Uint8List(bytes)..fillRange(0, bytes, 0xFF);

void main() {
  final since = DateTime.now().subtract(const Duration(days: 3));

  group('the round protocol runs in the documented order', () {
    test('start, fetch, data, ack — and the ack KEEPS the data on the band', () async {
      final rig = fetcherFor({
        0x13: [_stress([40, 41, 42])],
      });
      ...
      expect(commands.first, hasLength(10), reason: 'command + code + 8 time bytes');
      expect(commands.any((c) => c.length == 1 && c[0] == 0x02), isTrue);

      final acks = commands.where((c) => c.isNotEmpty && c[0] == 0x03).toList();
  ...
  group('the 0xFF gap-skip — the pager must cross a dead block, not stall', () {
    test('an all-sentinel stress round advances `since` by its minute count', () async {
      final rig = fetcherFor({
        0x13: [_allSentinel(90), _stress([50, 51])],
      });
    ...
    test('an all-sentinel activity round steps 8 bytes to the minute', () async {
      final rig = fetcherFor({
      ...
      expect(
        rig.strap.requestedSince[1].difference(rig.strap.requestedSince[0]).inMinutes,
        30,
    ...
    test('the skip is capped at a day, so one bad round cannot skip a year', () async {
      final rig = fetcherFor({
  ...
  group('paging forward through real data', () {
    test('the next round starts one minute after the last sample', () async {
      final rig = fetcherFor({
        0x13: [_stress([40, 41, 42]), _stress([43])],
      });
    ...
    test('maxRounds bounds the paging', () async {
      final rig = fetcherFor({
        0x13: [for (var i = 0; i < 10; i++) _stress([40, 41, 42])],
      });

      await rig.fetcher.fetchType(0x13, since, maxRounds: 2);
    ...
    test('lastRaw accumulates every round, not just the last', () async {
      final rig = fetcherFor({
        0x13: [_stress(List<int>.filled(20, 40)), _stress(List<int>.filled(15, 41))],
      });
  ...
  group('failures do not look like "no data"', () {
    test('a device that never answers times out and keeps what it had', () async {
      final fetcher = ActivityFetcher((_) async {});
      final samples = await fetcher.fetchType(
        0x13,
        since,
      ...
      expect(
        ...
        -1,
        reason: '-1 is "no reply", which is not the same as 0 = "nothing to send"',
      );
    });
    ...
    test('a control write that throws ends the job with a reason, not a hang', () async {
      final fetcher = ActivityFetcher((_) async {
        throw const FormatException('the radio is gone');
      });

      final samples = await fetcher
          .fetchType(0x13, since, timeout: const Duration(seconds: 2))
