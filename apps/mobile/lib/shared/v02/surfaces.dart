...
///
/// `.card .divider { margin-inline: calc(-1 * var(--space-xl)) }` makes the rule
/// reach both edges under the card's padding. Flutter has no negative padding,
/// so a card that needs the full-bleed rule is built `flush` and pads each of its
/// own children — which is what the CSS is doing, spelled out.
class CardDivider extends StatelessWidget {
  /// A rule with `margin-block: var(--space-lg)` above and below.
  const CardDivider({super.key});

  /// `.divider { margin-block: var(--space-lg) }`.
  static const double gap = 16;

  @override
  Widget build(BuildContext context) => Padding(
    padding: const EdgeInsets.symmetric(vertical: gap),
    child: Container(height: hairline, color: context.colors.line),
  );
}
