-- 0001_initial — the baseline Healthee schema.
--
-- Authoritative: this is what the migration runner (healthee.db.migrate) applies
-- to build a fresh database. `db/schema.sql` mirrors the resulting current schema
-- as an at-a-glance reference. All DDL is idempotent (IF NOT EXISTS / if_not_exists)
-- so a partial prior run is safe to replay.
--
-- Superset-compatible with legacy PROD (schema_v2.sql + every migration that ran:
-- programs, logging, weight_dedup, nap_kind, nap_unpollute, gps_track, gps_summary)
-- so the Phase 6 cutover can restore a pg_dump of prod into this schema. The v1
-- compat VIEWs (metric_sample, session) and v1-only tables (sync_run) are dropped
-- by design — the rebuild reads v2-native (INTELLIGENCE.md §6).
...
-- Primary read path: a metric's series over a time range, newest first.
