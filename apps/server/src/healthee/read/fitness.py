...
from healthee.read.common import USER_TZ, derived_series, latest_derived, sport_name, user_today
...
def vo2max_payload(cur: Cur) -> dict | None:
    ...
    cur.execute(
        "SELECT day, value, flags FROM derived_daily WHERE metric='vo2max_estimate' "
    ...
    return {
        "submax": _submax_block(cur, latest_value),
...
def _submax_block(cur: Cur, jurca_estimate: float) -> dict | None:
    """Submaximal HR-vs-pace VO2max from GPS workouts (``vo2max_submax``)."""
    cur.execute(
        "SELECT day, value, flags FROM derived_daily WHERE metric='vo2max_submax' "
        "AND day >= (current_date - 95) ORDER BY day",
    )
    rows = cur.fetchall()
    if not rows:
...
def cardio_load_payload(cur: Cur) -> dict | None:
    """Daily cardio load (Banister TRIMP) + strain 0-21 + 30-day trend/baseline.
    [[cardio_load_trimp]]."""
    cur.execute(
        "SELECT day, value, flags FROM derived_daily WHERE metric='cardio_load' "
    ...
    return {
        ...
        "strain": _strain(cur, latest_value),
        "strain_max": 21.0,
...
def _strain(cur: Cur, latest_load: float) -> float | None:
    ...
    cur.execute(
        ...
        "WHERE metric='cardio_load' AND value > 0 AND day >= (current_date - 90)",
    )
    row = cur.fetchone()
...
def _weekly_mvpa_rows(cur: Cur, days: int) -> list[tuple]:
    ...
    cur.execute(
        ...
        "WHERE metric='mvpa_min' AND day > (current_date - %s::int) ORDER BY day",
        (days,),
    )
    return cur.fetchall()


def mvpa_payload(cur: Cur) -> dict | None:
    """Weekly moderate-to-vigorous minutes vs the WHO 150-min target + 8-day
    breakdown. [[mvpa_minutes_mortality]], [[cadence_intensity]]."""
    rows = _weekly_mvpa_rows(cur, 8)
    if not rows:
        return None
    today = user_today()
...
def strength_payload(cur: Cur) -> dict:
    ...
    today = user_today()
    monday = today - timedelta(days=today.weekday())
    minutes, sessions, types = _strength_manual(cur, monday)
    m2, s2, t2 = _strength_workouts(cur, monday)
...
def _strength_manual(cur: Cur, monday) -> tuple[float, int, set[str]]:
    """Strength minutes from manual ``exercise`` entries since Monday (v2-native)."""
    cur.execute(
        "SELECT ts, end_ts, name, amount FROM manual_entry WHERE kind='exercise' AND ts >= %s",
        (_monday_utc(monday),),
    )
...
def _strength_workouts(cur: Cur, monday) -> tuple[float, int, set[str]]:
    """Strength minutes from strength-coded device ``workout`` rows since Monday."""
    cur.execute(
        "SELECT sport, duration_s FROM workout WHERE sport = ANY(%s) AND start_ts >= %s",
        (list(_STRENGTH_SPORTS), _monday_utc(monday)),
    )
    minutes, sessions, types = 0.0, 0, set()
...
def _monday_utc(monday: date) -> datetime:
    """UTC-aware instant for the start of the local week."""
    return datetime(monday.year, monday.month, monday.day, tzinfo=USER_TZ)
...
def fitness_plan_payload(cur: Cur) -> dict | None:
    ...
    for _d, mod, vig, _mv in _weekly_mvpa_rows(cur, (today - monday).days + 1):
        wk["moderate_min"] += mod
        wk["vigorous_min"] += vig
    return {
...
def activity_metric(cur: Cur, candidates: list[str], days: int = 30) -> dict | None:
    ...
    for cand in candidates:
        latest = latest_derived(cur, cand)
        if latest:
            ...
            return {
                ...
                "trend": derived_series(cur, cand, days),
            }
    return None


def workouts_list(cur: Cur, limit: int = 100) -> list[dict]:
    ...
    cur.execute(
        ...
        "FROM workout WHERE COALESCE(duration_s, 0) >= %s ORDER BY start_ts DESC LIMIT %s",
        (_MIN_WORKOUT_S, limit),
    )
    out = []
