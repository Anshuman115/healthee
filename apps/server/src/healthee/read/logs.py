...
def record_log(cur: Cur, req: LogRequest) -> dict:
    """Write one manual log to v2 tables. Returns ``{"ok": True}`` (with optional
    ``discarded``) or an explicit ``{"ok": False, "error": ...}`` for a rule outcome."""
    ts = datetime.fromtimestamp(req.at / 1000, tz=UTC) if req.at else datetime.now(tz=UTC)
    t = req.type
    if t == "weight":
        cur.execute(
            "INSERT INTO weight_log (ts, kg) VALUES (%s, %s) "
            "ON CONFLICT (ts) DO UPDATE SET kg = EXCLUDED.kg",
            (ts, float(req.amount or 0)),
        )
    elif t in _INSTANT_KINDS:
        cur.execute(
            "INSERT INTO manual_entry (kind, ts, name, amount, unit, notes) "
            "VALUES (%s,%s,%s,%s,%s,%s)",
            (t, ts, req.name, req.amount, req.unit, req.notes),
        )
    elif t in _DURATION_KINDS:
        mins = int(req.minutes or 0)
        cur.execute(
            "INSERT INTO manual_entry (kind, ts, end_ts, name, amount, unit, notes) "
            "VALUES (%s,%s,%s,%s,%s,'min',%s)",
            (t, ts - timedelta(minutes=mins), ts, req.kind, mins, req.notes),
        )
    elif t in ("fast_start", "fast_end"):
        return _record_fast(cur, t, ts, req.notes)
    else:
        return {"ok": False, "error": f"unknown log type {t}"}
    return {"ok": True}


def _record_fast(cur: Cur, t: str, ts: datetime, notes: str | None) -> dict:
    """Open/close a fasting entry (one open fast at a time; discard <5-min accidents)."""
    if t == "fast_start":
        cur.execute("SELECT 1 FROM manual_entry WHERE kind='fasting' AND end_ts IS NULL LIMIT 1")
        if cur.fetchone():
            return {"ok": False, "error": "a fast is already open"}
        cur.execute(
            "INSERT INTO manual_entry (kind, ts, notes) VALUES ('fasting', %s, %s)", (ts, notes)
        )
        return {"ok": True}
    cur.execute(
    ...
    if open_row and (ts - open_row[0]).total_seconds() < 300:
        cur.execute("DELETE FROM manual_entry WHERE kind='fasting' AND end_ts IS NULL")
        return {"ok": True, "discarded": "fast under 5 minutes — not recorded"}
    cur.execute("UPDATE manual_entry SET end_ts=%s WHERE kind='fasting' AND end_ts IS NULL", (ts,))
    return {"ok": True}
