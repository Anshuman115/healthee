...
def ingest_gps_track(cur: Cur, req: GpsTrackIn) -> dict:
    """Store a track + its points, derive submax VO2max, denormalise the summary."""
    pts = [
        p
        for p in req.points
    ...
    cur.execute(
        "INSERT INTO gps_track (start_ts, end_ts, source) VALUES (%s, %s, 'phone') RETURNING id",
        (datetime.fromisoformat(req.start_iso), datetime.fromisoformat(req.end_iso)),
    )
    ...
    track_id = inserted[0]
    cur.executemany(
        "INSERT INTO gps_point (track_id, ts, lat, lng, ele_m) "
        "VALUES (%s, to_timestamp(%s), %s, %s, %s) ON CONFLICT DO NOTHING",
        [(track_id, p[0], p[1], p[2], (p[3] if len(p) > 3 else None)) for p in pts],
    )
    vo2 = derive_vo2max_submax(cur, track_id)
    _denormalise_summary(cur, track_id, vo2)
    return {"ok": True, "track_id": str(track_id), "points": len(pts), "vo2max": vo2}


def _denormalise_summary(cur: Cur, track_id: str, vo2: dict) -> None:
    ...
    cur.execute(
        ...
        "vo2max_submax=%s, r2=%s WHERE id=%s",
        (
