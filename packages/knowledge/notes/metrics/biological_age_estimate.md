...
- **The fitness term is REQUIRED — no current VO₂max, no number.** The Stage-1 directive above ("hold the number back … until the VO₂max estimate … exist[s]") is enforced structurally, not by caveat. A term simply left out of `chrono + ΣΔAge` is not an omission: it is the assertion `HR_fitness = 1.0`, i.e. *this person is exactly at the age/sex-median VO₂max* — the dominant term, silently invented. So when the owner has no `vo2max_estimate` for their **own today** (withheld by [[non_exercise_vo2max]]'s gate, or not derived yet), `biological_age` and `delta_years` are `null`, `data_confidence` is `insufficient_data`, and `withheld` carries the VO₂max reason verbatim (`derive/vo2max.py::WITHHOLD_MESSAGES`) plus what the absence costs. The sleep-duration and regularity contributions still ship: each is a standalone hazard→years fact, and hiding them would withhold something we do know. The freshness rule is `derive/vo2max.py::estimate_unavailable_reason`, shared with `read/vo2max.py`, so the VO₂max card and this estimate can never disagree about whether today has a fitness number.
