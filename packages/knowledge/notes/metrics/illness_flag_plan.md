...
- **Status: SHIPPED (this plan is implemented) — as of 2026-08-01, and only half-true before then.** This line previously claimed the plan was implemented when only the READING half existed: `read/health_metrics.py::illness_flag_payload` (the Today pill), `read/recovery.py` (the guidance override), `challenges/recovery_guard.py` ([[recovery_readiness]] D7's hard training override) and `challenges/confounds.py` (the outcome ledger's illness-day count) all read `illness_flag`, while **nothing anywhere computed or inserted a row** — every `INSERT` in the repo was in a test. The table was empty in production, so the pill never rendered, the ledger always counted zero illness days, and a safety-critical override had a dead input. The producer is `derive/illness.py::derive_illness_flag`, run once per owner per local day as the FIRST step of `jobs/chain.py` (before `challenges`, which reads what it writes) and **ungated by entitlement** — `PRICING.md` §1a never paywalls safety.
- **Reading of step 1's ≥10-night gate:** the implementation applies it **per limb**, so an owner with no usable skin temperature still gets the strong, cited RR limb, and a night is skipped only when neither limb qualifies. This is what makes the Moderate tier's OR reachable; it is an interpretation of a line that can also be read as one joint gate, and it is recorded here rather than left implicit in the code.
- **Auto-clear is real, in two places:** re-running a day whose deltas have fallen back DELETES that day's row, and `read/health_metrics._ILLNESS_ACTIVE_DAYS` (2) ages a flag out when no new day qualifies. The latter stays the single definition of "active".
- **Inputs (v2 field names):** `respiratory_rate_sleep` (strong limb, a `derived_daily` metric) and `skin_temp_c` (weak limb, averaged over the **main** sleep session's window — it has no daily row) as personal-baseline deltas. `research_note_ids` on each flag cite only the limbs that actually produced a delta, so a chip never points at a signal we could not measure.
- **`hrv_delta_z` / `rhr_delta_z` are stored NULL** and nothing reads them. §Trigger logic step 4 makes HRV/RHR confidence *cross-checks, not thresholds*, and computing them would require inventing a **fourth** unsourced baseline window on top of the three [[respiratory_rate_normal]] already documents (14 / 42 / 30). Adding them is a deliberate future change, not an oversight.
- **The user-facing string** is produced by `_illness_framing` — **deterministic metric text, not an LLM** — e.g. "Possible early signal — consider lighter activity today. Breathing rate +X bpm vs your 14-day baseline; skin temperature +Y°C … Not a diagnosis," with the sustained-two-night ("Smarr 2020 / Quer 2021 pattern") suffix when applicable. It is rendered at READ time from the stored deltas; `framing` is not a column.
