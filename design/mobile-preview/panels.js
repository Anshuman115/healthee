...
(() => {
  ...
  H.bioHero = () => {
    ...
    return `<section class="bio-hero" data-tone="fitness">${H.charts.bioField()}<div class="bio-eyebrow"><span>Biological age · estimate</span><a href="#body" aria-label="Understand your biological age">${H.icon('arrow')}</a></div><div class="age-value">${age.biological_age}<small>years</small></div><p class="age-context"><strong>${Math.abs(age.delta_years)} years ${age.delta_years<0?'below':'above'}</strong> your chronological age of ${age.chronological_age}</p>${H.charts.ageScale()}<div class="bio-divider bio-bottom"><a href="#fitness"><span>Fitness contribution</span><strong>−1.7 years ↗</strong></a><a href="#sleep"><span>Sleep contribution</span><strong>0.0 years ↗</strong></a></div><span class="model-label">${H.icon('info','small')}Population-based model · not a clinical age</span></section>`;
  };
  ...
  H.overnightVitals = (n=H.demo.sleep.nights[0]) => {
    return `<div class="vitals-table">${[
      ['heart','Resting heart',n.rhr,'bpm','heart',H.demo.today.sparklines.rhr_daily.map(p=>p.value),[40,70],'rhr'],
      ['fitness','HRV',n.hrv_sleep_avg,'ms','activity',H.demo.today.sparklines.hrv_sleep_avg.map(p=>p.value),[25,65],'hrv'],
      ['oxygen','Blood oxygen',n.spo2_avg??'—','%','drop',H.demo.sleep.nights.slice(0,14).reverse().map(p=>p.spo2_avg),[90,100],'spo2'],
      ['oxygen','Breathing',n.respiratory_rate??'—','/min','activity',H.demo.today.sparklines.respiratory_rate_sleep.map(p=>p.value),[10,20],'breathing'],
      ['stress','Skin temperature',n.skin_temp_c??'—','°C','sun',H.demo.sleep.nights.slice(0,14).reverse().map(p=>p.skin_temp_c),[30,36],'temperature'],
