...
(() => {
  ...
  H.header = (title, subtitle = 'Friday, 31 July', detail = false) => `<header class="page-header ${detail ? 'detail' : ''}">${detail ? `<button class="icon-button" data-action="back" aria-label="Go back">${H.icon('back')}</button>` : ''}<div class="grow"><p class="date">${subtitle}</p><h1>${title}</h1></div>${detail ? '' : `<a href="#settings" class="icon-button avatar" aria-label="Profile and settings">A</a>`}</header>`;
