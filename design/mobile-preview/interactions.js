...
(() => {
  ...
  H.openLog = kind => {
    ...
    H.sheet(`Log ${name.toLowerCase()}`,`<form id="journal-form" data-kind="${name}" data-icon="${icon}" data-unit="${numeric?unit:''}">${valueField}<label class="field">When?<input type="datetime-local" name="time" value="2026-07-31T17:00" required></label><p class="form-note">Saved to the preview only. Nothing is uploaded.</p><button class="button full" type="submit">Save entry</button></form>`);
  };
